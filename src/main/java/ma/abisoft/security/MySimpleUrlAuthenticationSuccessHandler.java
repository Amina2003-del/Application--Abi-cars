...
public class MySimpleUrlAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
    ...
    protected String determineTargetUrl(final Authentication authentication) {
        ...
        if (authentication.getPrincipal() instanceof User) {
            ...
            for (Role role : roles) {
                Long roleId = role.getId(); // récupérer role_id

                if (roleId == 5) {  // client
                    return "/Clientes/rechercher-reserver";
                } else if (roleId == 4) {  // admin
                    return "/Administrateur/tableaubord";
                } else if (roleId == 6) {  // owner
        ...
        throw new IllegalStateException("Utilisateur sans rôle valide");
    }
